...
def evaluate_internal(y_pred, y_true, cxr_labels, plot_dir,
    ...
    for i in range(num_classes):
        ...
        results = [recall[0]]
        df = pd.DataFrame(results, columns=[cxr_label+'_recall'])
        dataframes.append(df)
        # """
    ...
    # f1_macro_df = pd.DataFrame([f1_macro], columns=['f1_macro'])
    # f1_micro_df = pd.DataFrame([f1_micro], columns=['f1_micro'])
    # f1_weighted_df = pd.DataFrame([f1_weighted], columns=['f1_weighted'])
