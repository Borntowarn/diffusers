...
def resize_array(array, current_spacing, target_spacing):
    """Изменение размера массива с учетом пространственного разрешения.
    
    Вычисляет новые размеры на основе соотношения текущего и целевого разрешения,
    затем применяет трилинейную интерполяцию для изменения размера массива.
    
    Args:
        array (torch.Tensor): Исходный массив для изменения размера
        current_spacing (tuple): Текущее пространственное разрешение (z, x, y)
        target_spacing (tuple): Целевое пространственное разрешение (z, x, y)
        
    Returns:
        np.ndarray: Измененный по размеру массив
    """
    ...
    def check_series_body_part(self, series):
        ...
        try:
            with TemporaryDirectory() as archive_temp_dir:
                ...
                result = self.model(dicom_array[None], np.array(dicom_spacings, dtype=np.float32)[None])[0]
                tag = result[0].decode("utf-8")
        except Exception as e:
            logger.error(f"Ошибка при определении области тела на серии: {e}")
            tag = ""

        logger.info(f"Область тела на серии из модели: {tag}")
        return tag
